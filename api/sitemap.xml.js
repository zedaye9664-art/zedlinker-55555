export default async function handler(req, res) {
  try {
    ...
    blogs.forEach(doc => {
      ...
      xml += `  <url><loc>https://www.zedlinker.store/blog/${name}</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>\n`;
    });

    const links = await fetchCol('links');
    links.forEach(doc => {
      const name = doc.name.split('/').pop();
      xml += `  <url><loc>https://www.zedlinker.store/link/${name}</loc><changefreq>daily</changefreq><priority>0.9</priority></url>\n`;
    });
